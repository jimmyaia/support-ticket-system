...
class SDKServer {
  ...
  // NOTE: authenticateRequest is no longer used for the main auth flow.
  // The standalone email/password auth is handled by authService.ts.
  // This method is kept only for cron/heartbeat compatibility.
  async authenticateRequest(req: Request): Promise<AuthenticatedUser> {
    ...
    if (!sessionToken) throw ForbiddenError("No session token");

    // Check if it's a cron token
    try {
      const userInfo = await this.getUserInfoWithJwt(sessionToken);
      if (userInfo.openId?.startsWith(CRON_OPEN_ID_PREFIX)) {
        const taskUid = userInfo.taskUid ?? null;
        if (!taskUid) throw ForbiddenError("Cron session missing task_uid");
        return buildCronUser(userInfo);
      }
    } catch {
      // Not a cron token, fall through
    }
    throw ForbiddenError("Use authService.authenticateRequest for regular users");
  }
}
